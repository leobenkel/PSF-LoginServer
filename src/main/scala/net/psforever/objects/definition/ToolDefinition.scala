...
class ToolDefinition(objectId: Int) extends EquipmentDefinition(objectId) {
  ...
  private def RepairMultipliers: Seq[Float] = repairMultipliers.toSeq

  private def RepairMultiplier(level: Int): Float = {
  ...
  private def AddRepairMultiplier(level: Int, value: Float): Seq[Float] = {
    if (level > 0) {
  ...
  private def DefaultFireModeIndex_=(index: Int): Int = DefaultFireModeIndex_=(Some(index))

  private def DefaultFireModeIndex_=(index: Option[Int]): Int = {
